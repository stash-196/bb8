...
def extraction(frame1, range_bb8, range_obs, range_goal):  # frame1: カメラから読み取られた画像をresizeした画像
    ...
    # BB8の輪郭とその外接円の描画
    if len(bb8) == 1:
        prev_frame = cv2.drawContours(frame1, bb8, -1, (0, 0, 255), 3)  # bb8の輪郭を赤色でframe1に追加
        for cnt in bb8:
            (x, y), radius = cv2.minEnclosingCircle(cnt)
            center = (int(x), int(y))
            radius = int(radius)
        # print('<<BB8>> 面積：{} 座標：{}'.format(bb8_areas, center))
        prev_frame = cv2.circle(prev_frame, center, 5, (255, 51, 51), -1) # 外接円をbb8の輪郭画像に追加しprev_frameを更新
        # 描画用
    ...
    if len(goal) != 0:
        ...
        for cnt in goal:
            for p in cnt:
                prev_frame = cv2.circle(goal_frame, (p[0][0], p[0][1]), 8, (0, 255, 255), 4)
            #  全体の描画
    cv2.imshow('Extraction Frame', prev_frame)
...
while True:
    ...
    frame1 = frame1[0:540, 280:853]  # 壁が入らないよいうにframe画像の左側280pxを切り取る
    range_bb8 = [300, 400]  # range_: それぞれの物体の面積の閾値
    range_obs = [5000, 6500]
    range_goal = [3000, 4990]
    # 関数呼び出し
